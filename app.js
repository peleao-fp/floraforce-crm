...
async function loadLeads() {
  ...
  while (true) {
    ...
    from += PAGE_SIZE;
    setLoader(`Carregando leads... ${allLeads.length}`, 50 + Math.min(20, allLeads.length / 500));
  }
  ...
  leads = allLeads.map(l => ({
    ...
    // state fields (will be overwritten by lead_states)
...
async function logActivity(leadId, leadName, action, detail) {
  await sb.from('activity_log').insert({ user_id: currentUser?.id, user_name: currentProfile?.name || '?', lead_id: leadId, lead_name: leadName, action, detail });
}
...
function renderTable() {
  ...
  if (!filteredLeads.length) { tbody.innerHTML = '<tr><td colspan="9"><div class="empty-state"><h3>Nenhum lead encontrado</h3></div></td></tr>'; return; }
  ...
  document.getElementById('modal-body').innerHTML = `
    ...
    <div class="modal-section"><div class="modal-section-title">Contato</div>
      <div class="info-grid">
        ...
        ${lead.ph ? `<div class="info-item"><div class="info-item-lbl">WhatsApp</div><div class="info-item-val"><a href="https://wa.me/${lead.ph.replace(/\D/g,'')}" target="_blank">Abrir ↗</a></div></div>` : ''}
      </div></div>
    ...
    <div class="modal-section"><div class="modal-section-title">Timeline</div>
      <div class="tl-add"><textarea id="tl-new-text" placeholder="Adicionar comentário..."></textarea>
        <button class="btn btn-primary" style="height:60px" onclick="addTimelineEntry()">+ Add</button></div>
...
function renderTimeline() {
  ...
  el.innerHTML = '<div class="timeline">' + [...e].reverse().map((x, ri) => {
    ...
    const deleteBtn = isAdmin ? `<button onclick="deleteTimelineEntry(${realIdx})" style="background:none;border:none;color:var(--danger);cursor:pointer;font-size:11px;padding:2px 6px;border-radius:4px;opacity:.7" title="Remover comentário">🗑</button>` : '';
    return `<div class="tl-entry"><div class="tl-dot"></div><div class="tl-body"><div class="tl-meta"><span class="tl-vendor">👤 ${esc(x.v || '—')}</span><span class="tl-date">${new Date(x.ts).toLocaleString('pt-BR')}</span>${deleteBtn}</div><div class="tl-text">${esc(x.txt)}</div></div></div>`;
  }).join('') + '</div>';
}

async function deleteTimelineEntry(idx) {
  if (!currentLead || currentProfile?.role !== 'admin') return;
  if (!confirm('Remover este comentário da timeline?')) return;
...
function setStatus(s) { if (!currentLead) return; const old = currentLead.cs; currentLead.cs = s; document.querySelectorAll('#modal-status-row .status-btn').forEach(b => b.classList.toggle('sel', b.getAttribute('onclick').includes("'" + s + "'"))); if (old !== s) logActivity(currentLead.id, currentLead.c, 'status_change', `${old} → ${s}`); }
async function registerCall() { if (!currentLead) return; currentLead.cc = (currentLead.cc || 0) + 1; currentLead.lc = new Date().toISOString(); if (currentLead.cs === 'novo') currentLead.cs = 'contatado'; sessionCalls++; updateProgress(); saveCallCount(); logActivity(currentLead.id, currentLead.c, 'call', `Ligação #${currentLead.cc}`); showToast('📞 Registrada!'); const cl = document.querySelector('.call-log'); if (cl) cl.innerHTML = `📞 Total: <strong style="color:var(--accent)">${currentLead.cc}</strong> · Última: <strong>${new Date(currentLead.lc).toLocaleString('pt-BR')}</strong>`; const bu = document.getElementById('btn-undo'); if (bu) { bu.disabled = false; bu.style.opacity = '1'; } }
async function undoCall() { if (!currentLead || !currentLead.cc) return; currentLead.cc--; if (!currentLead.cc) currentLead.lc = null; if (sessionCalls > 0) sessionCalls--; updateProgress(); saveCallCount(); showToast('↩ Desfeita'); }
...
function renderUsersTable() {
  ...
  tbody.innerHTML = allUsers.map(u => `<tr><td style="font-weight:500">${esc(u.name)}</td><td><span class="role-badge ${u.role === 'admin' ? 'role-admin' : 'role-vendor'}">${u.role}</span></td><td style="color:var(--accent);font-weight:600">${lpu[u.name] || 0}</td></tr>`).join('');
}
function renderKanban() {
  const vendors = allUsers.filter(u => u.role === 'vendor');
  const kanbanData = {};
  vendors.forEach(u => { kanbanData[u.name] = leads.filter(l => (l.responsible || l.r) === u.name).slice(0, 15); });
...
async function onDrop(e, vendor) {
  ...
  await saveLeadState(lead); logActivity(lead.id, lead.c, 'transfer', `${old} → ${vendor}`);
...
async function createUser() {
  ...
  showToast('✅ Usuário criado!'); await loadAdminData();
}
