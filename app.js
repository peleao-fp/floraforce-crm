...
async function doLogin() {
  ...
  if (error) { err.textContent='Incorrect email or password.'; err.style.display='block'; btn.disabled=false; btn.textContent='Entrar'; return; }
  document.getElementById('login-screen').style.display='none';
...
function renderTable() {
  ...
  if (!filteredLeads.length) { tbody.innerHTML='<tr><td colspan="10"><div class="empty-state"><h3>No leads found</h3></div></td></tr>'; return; }
...
function openModal(id) {
  ...
  const lastContactInfo = days===null
    ...
    : days===0 ? '<span style="color:var(--accent)">Contacted today</span>'
    : `<span style="color:${days>7?'var(--danger)':'var(--warn)'}">Last contact: <strong>${days} dia${days>1?'s':''} atrás</strong></span>`;
  ...
  document.getElementById('modal-body').innerHTML=`
    ...
    <div class="modal-section"><div class="modal-section-title">🔵 Status</div>
      <div class="status-row" id="modal-status-row">
        ${['novo','contatado','proposta','cliente'].map(s=>`<button class="status-btn ${lead.cs===s?'sel':''}" onclick="setStatus('${s}')">${{novo:'🔵 New',contatado:'🟡 Contacted',proposta:'🟣 Proposal',cliente:'🟢 Cliente'}[s]}</button>`).join('')}
      </div></div>
    <div class="modal-section"><div class="modal-section-title">📝 Lead Information</div>
      <div class="info-grid" id="lead-info-grid">
        ${renderEditableFields(lead)}
      </div></div>
    <div class="modal-section"><div class="modal-section-title">⭐ Priority</div>
      <label class="priority-toggle"><input type="checkbox" id="modal-priority" ${lead.pr?'checked':''} onchange="currentLead.pr=this.checked"><span>Mark as priority ⭐</span></label></div>
    <div class="modal-section"><div class="modal-section-title">🏷 Tags</div>
      <div class="tags-input-row">
        <input type="text" class="tags-input" id="tag-input" placeholder="Add tag..." onkeydown="if(event.key==='Enter'){addTag();event.preventDefault()}">
    ...
    <div class="modal-section"><div class="modal-section-title">📄 Notes</div>
      <textarea class="modal-textarea" id="modal-comments" placeholder="General notes...">${esc(lead.cm||'')}</textarea></div>
    <div class="modal-section"><div class="modal-section-title">📞 Calls</div>
      <div class="call-log">Total: <strong style="color:var(--accent)">${lead.cc||0}</strong>${lead.lc?` · Last: <strong>${new Date(lead.lc).toLocaleString('pt-BR')}</strong>`:' · None'}</div></div>
    <div class="modal-section"><div class="modal-section-title">💬 Comment Timeline</div>
      <div class="tl-add">
        <textarea id="tl-new-text" placeholder="Add comment..."></textarea>
...
function renderEditableFields(lead) {
  ...
  return field('Company',    'c',  lead.c)
    ...
    + `<div class="info-item">
      ...
      <div class="info-item">
        <div class="info-item-lbl">State</div>
...
function addTimelineEntry() {
  ...
  if(!txt){showToast('⚠️ Type a comment');return;}
...
async function registerCall(){
  ...
  if(cl) cl.innerHTML=`Total: <strong style="color:var(--accent)">${currentLead.cc}</strong> · Last: <strong>${new Date(currentLead.lc).toLocaleString('pt-BR')}</strong>`;
...
function updateCallCount(val){
  ...
  updateProgress();
  saveCallCount();
  logActivity(currentLead.id,currentLead.c,'call_edit',`Calls: ${old} → ${newVal}`);
  showToast(`✏️ Calls: ${newVal}`);
}
...
async function saveModal(){
  ...
  await saveLeadState(currentLead);
  closeModal(); applyFilters(); showToast('✅ Saved!');
}
...
function renderDashboard(){
  ...
  document.getElementById('kpi-grid').innerHTML=`
    ...
    <div class="kpi-card purple"><div class="kpi-card-val">${withPhone.toLocaleString()}</div><div class="kpi-card-lbl">Com Phone</div></div>
    <div class="kpi-card danger"><div class="kpi-card-val">${noContact}</div><div class="kpi-card-lbl">Never Contacteds</div></div>
  ...
  document.getElementById('charts-row').innerHTML=`
    ...
    <div class="chart-card"><div class="chart-title">💰 Receita por Owner</div><div class="bar-chart">${repE.map(([r,v])=>`<div class="bar-row"><div class="bar-label">${r||'—'}</div><div class="bar-track"><div class="bar-fill" style="width:${(v/maxRep*100).toFixed(0)}%"></div></div><div class="bar-val">$${(v/1000).toFixed(0)}K</div></div>`).join('')}</div></div>`;
}
...
function renderKanban(){
  ...
  if(unas.length) kd['Unassigned']=unas;
...
async function createUser(){
  ...
  showToast('✅ User created! Email: '+email);
