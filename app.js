...
function renderOwnerDropdown(currentOwner) {
  ...
  // Non-admins see a read-only field
  if (!isAdmin) {
  ...
  // Admins get the full dropdown with all profiles
...
async function renderMktTagDropdown(lead) {
  // Always fetch fresh from Supabase so all users see the latest tags
  await loadMktTagTypes();
...
function exportCSV() {
  ...
  const pool = isAdmin ? leads : getMyLeads();
  const rows = [
    ['ID', 'Company', 'Contact', 'Email', 'Phone', 'Owner', 'Pipeline', 'Type', 'Status', 'State', 'Calls', 'Last Contact', 'Tags', 'MKT Tag']
  ];
  pool.forEach(l => {
    rows.push([
      l.id,
...
function parseBulkCSV(text) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    check('p',           get(colMap.pipeline), lead.p,                     'Pipeline');
...
const ALL_PERMISSIONS = [
  { key: 'can_export_csv',   label: '📥 CSV Export' },
  { key: 'can_view_analytics', label: '📈 Analytics' },
  { key: 'can_create_quotes',  label: '📄 Quotes' },
  { key: 'can_bulk_import',    label: '📦 Bulk Import' },
  { key: 'can_view_mkt',       label: '📧 MKT' },
];

function hasPermission(key) {
  if (currentProfile?.role === 'admin') return true;
  return currentProfile?.permissions?.[key] === true;
}
...
function renderPermissionsTable() {
  ...
  if (!vendors.length) { tbody.innerHTML = '<tr><td colspan="6" style="color:var(--text3);text-align:center;padding:20px">No vendors found</td></tr>'; return; }

  tbody.innerHTML = vendors.map(u => {
    const perms = u.permissions || {};
    const toggles = ALL_PERMISSIONS.map(p =>
      '<td style="text-align:center">'
      + '<label class="perm-toggle" title="' + p.label + '">'
      + '<input type="checkbox" ' + (perms[p.key] ? 'checked' : '') + ' onchange="togglePermission(\'' + u.id + '\',\'' + p.key + '\',this.checked)">'
      + '<span class="perm-toggle-slider"></span>'
      + '</label></td>'
    ).join('');
    return '<tr><td style="font-weight:500">' + esc(u.name) + '<br><span style="font-size:10px;color:var(--text3)">' + u.role + '</span></td>' + toggles + '</tr>';
  }).join('');
}
...
async function togglePermission(userId, permKey, value) {
  ...
  if (error) { showToast('❌ Error: ' + error.message); return; }
  showToast('✅ Permission updated for ' + user.name);
