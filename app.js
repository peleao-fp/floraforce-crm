...
async function loadCallCount() {
  ...
  if (data) {
    const profileLower = currentProfile.name.toLowerCase();
    sessionCalls = data.filter(c => {
      if (!c.user_name) return false;
      const iLower = c.user_name.toLowerCase();
      const words = iLower.split(' ').filter(w => w.length > 3);
      return profileLower.includes(iLower) || iLower.includes(profileLower) ||
             words.some(w => profileLower.includes(w));
    }).length;
  } else {
    sessionCalls = 0;
  }
...
function renderPipelineDropdown(lead) {
  // Get unique pipelines from loaded leads
...
async function updateLeadPipeline(select) {
  ...
  // Update in leads table directly (pipeline lives in leads table, not lead_states)
...
async function updateLeadField(input) {
  ...
  if (key === 'responsible') {
    ...
    // Save to both leads table AND lead_states
    // Use empty string for unassigned (not null, to avoid RLS issues)
...
async function registerCall() {
  ...
  // call count tracked via Intermedia sync
...
function renderDashboard() {
  ...
  // Use Intermedia-based call counts for dashboard
...
async function loadAdminData() {
  ...
  allUsers = data || [];
  // Rebuild call counts now that allUsers is populated
...
function renderUsersTable() {
  ...
  // Use Intermedia-based call counts (with partial name matching)
...
async function loadAnalytics() {
  ...
  // ── Build vendor stats ──
  // Normalize vendor name: map partial/old names to canonical profile names
  function canonicalName(raw) {
    if (!raw) return 'Unassigned';
    const r = raw.trim();
    // Match against known profiles by partial name
    if (allUsers.length) {
  ...
  // Use Intermedia call log for the selected period (accurate, date-filtered)
  calls.forEach(c => {
...
function exportCSV() {
  ...
  const pool = isAdmin ? leads : getMyLeads();
  const rows = [
    ['Company', 'Contact', 'Email', 'Phone', 'Owner', 'Pipeline', 'Type', 'Status', 'State', 'Calls', 'Last Contact', 'Tags']
  ];
  pool.forEach(l => {
    rows.push([
      l.c || '',
...
function refreshMktPreview() {
  ...
  const preview = withEmail.slice(0, 50);
  if (!preview.length) {
    tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;color:var(--text3);padding:20px">No leads with email match this filter</td></tr>';
    return;
  }
  tbody.innerHTML = preview.map(l =>
    '<tr>'
    + '<td>' + esc(l.c || '—') + '</td>'
    + '<td>' + esc(l.cn || '—') + '</td>'
...
async function exportToMailchimp() {
  ...
  try {
    ...
    // Batch upload members in chunks of 500
