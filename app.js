...
function renderDashboard() {
  ...
  document.getElementById('kpi-grid').innerHTML =
    '<div class="kpi-card green"><div class="kpi-card-val">$' + (totalRev/1000).toFixed(0) + 'K</div><div class="kpi-card-lbl">' + (isAdmin ? 'Total' : 'My') + ' Revenue</div></div>'
...
async function loadAnalytics() {
  ...
  document.getElementById('analytics-kpis').innerHTML =
    kpi('green',  '$'+(totalRevenue/1000).toFixed(1)+'K', 'Total Revenue')
  ...
  document.getElementById('analytics-charts-1').innerHTML =
    '<div class="chart-card"><div class="chart-title">💰 Revenue by Vendor</div><div class="bar-chart">'
    + vList.slice(0,8).map(v =>
        '<div class="bar-row"><div class="bar-label">'+esc(v.name)+'</div>'
        +'<div class="bar-track"><div class="bar-fill" style="width:'+(v.revenue/maxRev*100).toFixed(0)+'%"></div></div>'
        +'<div class="bar-val">$'+(v.revenue/1000).toFixed(1)+'K</div></div>'
      ).join('') + '</div></div>'
    + '<div class="chart-card"><div class="chart-title">📞 Calls by Vendor</div><div class="bar-chart">'
    + vList.slice(0,8).map(v =>
        '<div class="bar-row"><div class="bar-label">'+esc(v.name)+'</div>'
  ...
  const maxVRev  = Math.max(...vList.map(v=>v.revenue), 1);
  ...
  document.getElementById('vendor-tbody').innerHTML = vList.map(v => {
    ...
    const barW = (v.revenue/maxVRev*60).toFixed(0);
    return '<tr>'
      ...
      + '<td><span class="perf-bar" style="width:'+barW+'px"></span>'+fmtMoney(v.revenue)+'</td>'
      + '<td class="num conv-rate '+crClass+'">'+cr+'%</td>'
      + '</tr>';
  }).join('') || '<tr><td colspan="8" style="text-align:center;color:var(--text3);padding:20px">No data</td></tr>';
}
...
function switchTab(tab, el) {
  ...
  if (tab === 'analytics' && !analyticsDateFrom) {
    // Trigger default 30-day preset on first open
