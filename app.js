...
async function loadIntermedaCallCounts() {
  const { data } = await sb.from('intermedia_call_log')
    ...
    .not('lead_id', 'is', null);
  if (!data) return;
  const counts = {}, lastCall = {};
  // Build global call stats by Intermedia user_name (for admin panel)
  const callsByIntermediaName = {};
  data.forEach(c => {
    if (!c.lead_id) return;
  ...
  // Map Intermedia names → profile names (partial match, case-insensitive)
  ...
  allUsers.forEach(u => {
    ...
    Object.entries(callsByIntermediaName).forEach(([iName, cnt]) => {
      ...
      // Match if names share significant words (3+ chars)
...
async function loadCallCount() {
  ...
  // Fetch all calls this week then match by partial name
  // (Intermedia may use "Gabriel Campos Sobrinho" vs profile "S13 GABRIEL CAMPOS SOBRINHO")
  const { data } = await sb.from('intermedia_call_log')
    .select('user_name')
...
async function updateLeadField(input) {
  ...
  if (key === 'responsible') {
    ...
    await sb.from('leads').update({ responsible: newVal || null }).eq('id', currentLead.id);
...
async function loadAdminData() {
  ...
  loadCallsLog();
  loadActivityLog();
  // Re-render users table after intermedia counts are populated
  if (!window._callsByProfile) {
    await loadIntermedaCallCounts();
    renderUsersTable();
  }
}
