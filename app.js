...
async function boot() {
  loadTheme();
  initSB();
  setLoader('Verificando sessão...', 20);
...
async function loadApp(user) {
  currentUser = user;
  setLoader('Carregando perfil...', 30);
  const { data: profile } = await sb.from('profiles').select('*').eq('id', user.id).single();
  currentProfile = profile;
  setLoader('Carregando leads...', 50);
  await loadLeads();
  setLoader('Carregando estados...', 75);
  ...
  setTimeout(() => {
    ...
    document.getElementById('user-role').textContent = currentProfile?.role === 'admin' ? 'Admin' : 'Vendedor';
    populateFilters();
    applyFilters();
    renderDashboard();
    startIdleTracking();
    logActivity(null, null, 'login', `Entrou no sistema`);
  }, 300);
}
...
function startIdleTracking() {
  const reset = () => {
    if (Date.now() - lastActivity > IDLE_MINUTES * 60 * 1000) {
      logActivity(null, null, 'idle_return', 'Voltou após inatividade');
    }
    lastActivity = Date.now();
    clearTimeout(idleTimer);
    idleTimer = setTimeout(() => {
      logActivity(null, null, 'idle', `Inativo por ${IDLE_MINUTES}min`);
...
async function loadLeads() {
  ...
  while (true) {
    ...
    from += PAGE_SIZE;
    setLoader(`Carregando leads... ${all.length}`, 55);
  }
  leads = all.map(l => ({
    id: l.id, c: l.company || '', p: l.pipeline || '', r: l.responsible || '',
    st: l.state || '', ty: l.type || '', cn: l.contact || '',
    em: l.email || '', ph: l.phone || '',
    sl: l.sales_total ? { total: parseFloat(l.sales_total)||0, count: parseInt(l.sales_count)||0, rep: l.sales_rep||'', last_date: l.sales_last||'' } : null,
    cs: 'novo', tg: [], pr: false, cc: 0, lc: null, cv: false, cm: '', tl: [],
...
async function loadCallCount() {
  ...
  const { data } = await sb.from('call_counts').select('calls').eq('user_id', currentUser.id).eq('week_key', getWeek()).single();
  sessionCalls = data?.calls||0;
  updateProgress();
}
async function saveCallCount() {
  if (!currentUser) return;
  await sb.from('call_counts').upsert({ user_id: currentUser.id, vendor_name: currentProfile?.name||'', week_key: getWeek(), calls: sessionCalls, updated_at: new Date().toISOString() }, { onConflict: 'user_id,week_key' });
}

// ── ACTIVITY LOG ─────────────────────────────────────────────
async function logActivity(leadId, leadName, action, detail) {
  await sb.from('activity_log').insert({
    user_id: currentUser?.id, user_name: currentProfile?.name||'?',
    lead_id: leadId, lead_name: leadName, action, detail
  });
}
...
async function doLogout() {
  logActivity(null, null, 'logout', 'Saiu do sistema');
...
function lastContactBadge(lead) {
  ...
  if (days === null) return '<span style="color:var(--text3);font-size:10px">Nunca</span>';
  if (days === 0)    return '<span style="color:var(--accent);font-size:10px">Hoje</span>';
...
function intermediaLink(phone) {
  if (!phone) return '';
  const clean = phone.replace(/\D/g,'');
  // Intermedia Unite uses tel: protocol which opens the app's dialpad
  return `<a href="tel:${clean}" title="Ligar via Intermedia" style="text-decoration:none">📞</a>`;
}

function statusBadge(cs) {
  return {
    novo:      '<span class="badge badge-novo">Novo</span>',
    contatado: '<span class="badge badge-contatado">Contatado</span>',
    proposta:  '<span class="badge badge-proposta">Proposta</span>',
    cliente:   '<span class="badge badge-cliente">Cliente ✓</span>'
  }[cs] || '<span class="badge">—</span>';
}
...
function renderTable() {
  ...
  if (!filteredLeads.length) { tbody.innerHTML='<tr><td colspan="10"><div class="empty-state"><h3>Nenhum lead encontrado</h3></div></td></tr>'; return; }
  tbody.innerHTML=page.map(l => {
    ...
    const callBtn = l.ph
      ? `<a href="tel:${l.ph.replace(/\D/g,'')}" class="btn btn-ghost" style="padding:4px 8px;font-size:11px;text-decoration:none" title="Ligar: ${esc(l.ph)}">📞</a>`
...
function openModal(id) {
  ...
  const salesHtml=lead.sl
    ?`<div class="sales-card"><div class="sales-card-val">$${lead.sl.total.toFixed(2)}</div><div class="sales-card-sub">${lead.sl.count} pedidos · Rep: ${lead.sl.rep} · Último: ${lead.sl.last_date}</div></div>`
  ...
  const lastContactInfo = days===null
    ? '<span style="color:var(--text3)">Nunca contactado</span>'
    : days===0 ? '<span style="color:var(--accent)">Contactado hoje</span>'
    : `<span style="color:${days>7?'var(--danger)':'var(--warn)'}">Último contato: <strong>${days} dia${days>1?'s':''} atrás</strong></span>`;

  const phoneHtml = lead.ph
    ? `<a href="tel:${lead.ph.replace(/\D/g,'')}" class="btn btn-primary" style="display:inline-flex;align-items:center;gap:6px;text-decoration:none;padding:8px 14px;font-size:12px" title="Abre o Intermedia">📞 Ligar: ${esc(lead.ph)}</a>`
    : '<span style="color:var(--text3);font-size:12px">Sem telefone cadastrado</span>';

  document.getElementById('modal-body').innerHTML=`
    <div class="modal-section"><div class="modal-section-title">📊 Histórico de Vendas</div>${salesHtml}</div>
    <div class="modal-section"><div class="modal-section-title">⏱ Último Contato</div><div style="padding:8px 0">${lastContactInfo}</div></div>
    <div class="modal-section"><div class="modal-section-title">📞 Ligar via Intermedia</div>${phoneHtml}</div>
    <div class="modal-section"><div class="modal-section-title">🔵 Status</div>
    ...
    <div class="modal-section"><div class="modal-section-title">📝 Informações do Lead</div>
      <div class="info-grid" id="lead-info-grid">
        ${renderEditableFields(lead)}
      </div></div>
    <div class="modal-section"><div class="modal-section-title">⭐ Prioridade</div>
      <label class="priority-toggle"><input type="checkbox" id="modal-priority" ${lead.pr?'checked':''} onchange="currentLead.pr=this.checked"><span>Marcar como prioridade ⭐</span></label></div>
    ...
    <div class="modal-section"><div class="modal-section-title">📄 Notas</div>
      <textarea class="modal-textarea" id="modal-comments" placeholder="Notas gerais...">${esc(lead.cm||'')}</textarea></div>
    <div class="modal-section"><div class="modal-section-title">📞 Ligações</div>
      <div class="call-log">Total: <strong style="color:var(--accent)">${lead.cc||0}</strong>${lead.lc?` · Última: <strong>${new Date(lead.lc).toLocaleString('pt-BR')}</strong>`:' · Nenhuma'}</div></div>
    <div class="modal-section"><div class="modal-section-title">💬 Timeline de Comentários</div>
      <div class="tl-add">
        <textarea id="tl-new-text" placeholder="Adicionar comentário..."></textarea>
        <button class="btn btn-primary" style="height:60px" onclick="addTimelineEntry()">+ Add</button>
      </div>
  ...
  document.getElementById('modal-footer').innerHTML=`
    <div style="display:flex;align-items:center;gap:6px;font-size:12px;color:var(--text3)">
      Ligações: <input type="number" id="call-count-edit" min="0" value="${lead.cc||0}" 
        style="width:55px;background:var(--bg);border:1px solid var(--border2);color:var(--text);border-radius:6px;padding:4px 7px;font-size:12px;text-align:center"
        onchange="updateCallCount(this.value)">
    </div>
...
function renderEditableFields(lead) {
  const field = (label, key, val, type='text') =>
    `<div class="info-item">
      <div class="info-item-lbl">${label}</div>
      <div class="info-item-val">
        <input type="${type}" class="edit-field" data-key="${key}" value="${esc(val||'')}" placeholder="${label}..." onchange="updateLeadField(this)">
      </div>
    </div>`;

  return field('Empresa',    'c',  lead.c)
    + field('Contato',      'cn', lead.cn)
    + field('Email',        'em', lead.em, 'email')
    + field('Telefone',     'ph', lead.ph, 'tel')
    + field('Responsável',  'responsible', lead.responsible||lead.r)
    + field('Tipo',         'ty', lead.ty)
    + `<div class="info-item">
        <div class="info-item-lbl">Pipeline</div>
        <div class="info-item-val" style="font-size:11px;color:var(--text3)">${esc(lead.p||'—')}</div>
      </div>
      <div class="info-item">
        <div class="info-item-lbl">Estado</div>
        <div class="info-item-val" style="font-size:11px;color:var(--text3)">${esc(lead.st||'—')}</div>
      </div>`;
}
...
function updateLeadField(input) {
  ...
  showToast(`✏️ ${key} atualizado`);
}

// ── TIMELINE ──────────────────────────────────────────────────
function renderTimeline() {
  const el=document.getElementById('timeline-list'); if(!el||!currentLead) return;
  const e=currentLead.tl||[];
  if(!e.length){el.innerHTML='<div class="tl-empty">Nenhum comentário.</div>';return;}
  const isAdmin=currentProfile?.role==='admin';
  el.innerHTML='<div class="timeline">'+[...e].reverse().map((x,ri)=>{
    const realIdx=(currentLead.tl.length-1)-ri;
    const delBtn=isAdmin?`<button onclick="deleteTimelineEntry(${realIdx})" style="background:none;border:none;color:var(--danger);cursor:pointer;font-size:11px;padding:2px 6px;border-radius:4px;opacity:.7" title="Remover">🗑</button>`:'';
    return `<div class="tl-entry"><div class="tl-dot"></div><div class="tl-body"><div class="tl-meta"><span class="tl-vendor">👤 ${esc(x.v||'—')}</span><span class="tl-date">${new Date(x.ts).toLocaleString('pt-BR')}</span>${delBtn}</div><div class="tl-text">${esc(x.txt)}</div></div></div>`;
  }).join('')+'</div>';
}
...
async function deleteTimelineEntry(idx) {
  ...
  logActivity(currentLead.id,currentLead.c,'comment_deleted',`Removeu: "${(removed?.txt||'').substring(0,50)}"`);
...
function setStatus(s){
  if(!currentLead)return;
  const old=currentLead.cs; currentLead.cs=s;
...
async function registerCall(){
  if(!currentLead)return;
  currentLead.cc=(currentLead.cc||0)+1; currentLead.lc=new Date().toISOString();
  if(currentLead.cs==='novo') currentLead.cs='contatado';
  sessionCalls++; updateProgress(); saveCallCount();
  logActivity(currentLead.id,currentLead.c,'call',`Ligação #${currentLead.cc}`);
  showToast('📞 Registrada!');
  const cl=document.querySelector('.call-log');
  if(cl) cl.innerHTML=`Total: <strong style="color:var(--accent)">${currentLead.cc}</strong> · Última: <strong>${new Date(currentLead.lc).toLocaleString('pt-BR')}</strong>`;
  const bu=document.getElementById('btn-undo'); if(bu){bu.disabled=false;bu.style.opacity='1';}
}

function updateCallCount(val){
  if(!currentLead) return;
  const old=currentLead.cc||0;
  const newVal=Math.max(0,parseInt(val)||0);
  currentLead.cc=newVal;
  if(newVal===0) currentLead.lc=null;
  const diff=newVal-old;
  sessionCalls=Math.max(0,sessionCalls+diff);
  updateProgress();
  saveCallCount();
  logActivity(currentLead.id,currentLead.c,'call_edit',`Ligações: ${old} → ${newVal}`);
  showToast(`✏️ Ligações: ${newVal}`);
}
async function undoCall(){
  if(!currentLead||!currentLead.cc) return;
  currentLead.cc--; if(!currentLead.cc) currentLead.lc=null;
  if(sessionCalls>0) sessionCalls--;
  updateProgress(); saveCallCount(); showToast('↩ Desfeita');
}

async function saveModal(){
  if(!currentLead) return;
  const cm=document.getElementById('modal-comments'); if(cm) currentLead.cm=cm.value;
  await saveLeadState(currentLead);
  closeModal(); applyFilters(); showToast('✅ Salvo!');
}
...
function renderDashboard(){
  ...
  const stale=pool.filter(l=>daysSince(l.lc)>7).length;
  document.getElementById('kpi-grid').innerHTML=`
    <div class="kpi-card green"><div class="kpi-card-val">$${(totalRev/1000).toFixed(0)}K</div><div class="kpi-card-lbl">Receita Março</div></div>
    <div class="kpi-card blue"><div class="kpi-card-val">${pool.length.toLocaleString()}</div><div class="kpi-card-lbl">Meus Leads</div></div>
    <div class="kpi-card yellow"><div class="kpi-card-val">${withSales}</div><div class="kpi-card-lbl">Com Compras</div></div>
    <div class="kpi-card purple"><div class="kpi-card-val">${withPhone.toLocaleString()}</div><div class="kpi-card-lbl">Com Telefone</div></div>
    <div class="kpi-card danger"><div class="kpi-card-val">${noContact}</div><div class="kpi-card-lbl">Nunca Contatados</div></div>
    <div class="kpi-card warn"><div class="kpi-card-val">${stale}</div><div class="kpi-card-lbl">Sem contato +7d</div></div>`;
  ...
  const byRep={}; pool.filter(l=>l.sl).forEach(l=>{byRep[l.responsible||l.r]=(byRep[l.responsible||l.r]||0)+l.sl.total;});
  const repE=Object.entries(byRep).sort((a,b)=>b[1]-a[1]).slice(0,6),maxRep=repE[0]?.[1]||1;
  document.getElementById('charts-row').innerHTML=`
    <div class="chart-card"><div class="chart-title">📊 Pipeline</div><div class="bar-chart">${pipeE.slice(0,6).map(([p,v])=>`<div class="bar-row"><div class="bar-label">${p.split(' - ').pop()}</div><div class="bar-track"><div class="bar-fill" style="width:${(v/maxPipe*100).toFixed(0)}%;background:linear-gradient(90deg,#a78bfa,#c4b5fd)"></div></div><div class="bar-val">${v}</div></div>`).join('')}</div></div>
    <div class="chart-card"><div class="chart-title">💰 Receita por Responsável</div><div class="bar-chart">${repE.map(([r,v])=>`<div class="bar-row"><div class="bar-label">${r||'—'}</div><div class="bar-track"><div class="bar-fill" style="width:${(v/maxRep*100).toFixed(0)}%"></div></div><div class="bar-val">$${(v/1000).toFixed(0)}K</div></div>`).join('')}</div></div>`;
}
...
async function loadAdminData(){
  ...
  allUsers=data||[]; renderUsersTable(); renderKanban(); loadActivityLog();
}
function renderUsersTable(){
  const tbody=document.getElementById('users-tbody');if(!tbody)return;
  const lpu={}; leads.forEach(l=>{const r=l.responsible||l.r;if(r)lpu[r]=(lpu[r]||0)+1;});
...
function renderKanban(){
  ...
  const unas=leads.filter(l=>{const r=l.responsible||l.r;return !r||!allUsers.find(u=>u.name===r);}).slice(0,15);
  if(unas.length) kd['Não atribuído']=unas;
  ...
  wrap.innerHTML=Object.entries(kd).map(([vendor,vl])=>`
    <div class="kanban-col" data-vendor="${esc(vendor)}">
      ...
      <div class="kanban-col-body" ondragover="onDragOver(event)" ondrop="onDrop(event,'${esc(vendor)}')" ondragleave="onDragLeave(event)">
        ${vl.map(l=>`<div class="kanban-card" draggable="true" ondragstart="onDragStart(event,${l.id})"><div class="kanban-card-name">${esc(l.c)}</div><div class="kanban-card-state">${{novo:'🔵',contatado:'🟡',proposta:'🟣',cliente:'🟢'}[l.cs]||'⚪'} ${l.cs}</div></div>`).join('')}
      </div></div>`).join('');
}
...
async function onDrop(e,vendor){
  ...
  renderKanban(); showToast(`✅ "${lead.c.substring(0,20)}" → ${vendor}`); draggingId=null;
}
async function loadActivityLog(){
  ...
  el.innerHTML=data.map(a=>`<div class="activity-item">
    <div class="act-icon">${icons[a.action]||'📝'}</div>
    <div class="act-body"><strong>${esc(a.user_name||'—')}</strong> · ${esc(a.action.replace(/_/g,' '))} ${a.lead_name?`em <strong>${esc(a.lead_name.substring(0,25))}</strong>`:''}${a.detail?`<br><span style="color:var(--text3)">${esc(a.detail.substring(0,80))}</span>`:''}</div>
...
async function createUser(){
  ...
  showToast('✅ Usuário criado! Email: '+email);
