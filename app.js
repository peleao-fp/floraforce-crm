...
function parseBulkCSV(text) {
  ...
  const colMap = {
    ...
    pipeline:   headers.findIndex(h => h === 'pipeline'),
  ...
  const statusMap = { 'new': 'novo', 'contacted': 'contatado', 'proposal': 'proposta', 'customer': 'cliente', 'novo':'novo','contatado':'contatado','proposta':'proposta','cliente':'cliente' };
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const get = idx => idx >= 0 && cols[idx] !== undefined ? cols[idx].replace(/^"|"$/g,'').trim() : null;

    const csvId       = get(colMap.id);
    const csvCompany  = get(colMap.company);

    // Find matching lead by ID first, then company name
    let lead = null;
    ...
    if (!lead) { bulkPreviewData.push({ error: `Not found: "${csvId || csvCompany}"`, row: i }); continue; }

    const changes = {};
    const display = [];

    const check = (field, csvVal, currentVal, label) => {
      if (csvVal !== null && csvVal !== '' && csvVal !== currentVal) {
        changes[field] = csvVal;
        display.push({ label, from: currentVal || '—', to: csvVal });
      }
    };
    ...
    if (csvTags !== null && csvTags !== '') {
      const newTags = csvTags.split(';').map(t => t.trim()).filter(Boolean);
      const currentTags = (lead.tg || []).join('; ');
      if (csvTags !== currentTags) {
        changes['tg'] = newTags;
        display.push({ label: 'Tags', from: currentTags || '—', to: csvTags });
      }
    }
