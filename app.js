...
async function loadIntermedaCallCounts() {
  // Get all call counts grouped by lead_id from intermedia_call_log
  const { data } = await sb.from('intermedia_call_log')
    .select('lead_id, called_at')
    .not('lead_id', 'is', null);
  if (!data) return;
  // Count per lead and find last call date
  ...
  leads.forEach(l => {
    if (counts[l.id]) {
      l.cc = counts[l.id];
      // Use most recent of lead_state last_call or intermedia last call
...
async function loadCallCount() {
  ...
  const { count } = await sb.from('intermedia_call_log')
    .select('id', { count: 'exact', head: true })
    .eq('user_name', currentProfile.name)
