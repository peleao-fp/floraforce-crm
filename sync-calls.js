...
async function getCallLogs(token) {
  ...
  const calls = Array.isArray(data) ? data
    : (data.calls || data.items || data.records || data.data || data.results || []);
...
async function getSyncedCallIds() {
  const res = await fetch(`${SUPABASE_URL}/rest/v1/intermedia_call_log?select=call_id&limit=10000`, {
    headers: {
      'apikey':        SUPABASE_SERVICE_KEY,
      'Authorization': `Bearer ${SUPABASE_SERVICE_KEY}`
    }
